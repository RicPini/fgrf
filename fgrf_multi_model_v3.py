...
2. Efficient Hausdorff dimension (participation ratio with correction)
...
Usage:
    python3 fgrf_multi_model_robust.py --model gpt2 --ablation full
    python3 fgrf_multi_model_robust.py --model Qwen/Qwen2.5-0.5B --dtype float16 --fast
...
def compute_topological_complexity(weight_matrix: torch.Tensor, k: float,
    ...
    """
    C(k) = d log N(1/k) / d log k

    Adaptive finite differences: the window is chosen from the local
    spacing of singular values around the threshold 1/k, so that
    N(1/k) actually changes across the window.
    """
    ...
    # Adaptive window: straddle the two singular values that bracket 1/k
    threshold = 1.0 / max(k, 1e-12)
    below = np.where(S > threshold)[0]
    if len(below) == 0:
        return 1.0  # everything below threshold
    n_above = len(below)

    # pick the boundary: largest index above threshold
    idx = n_above - 1  # S[idx] > threshold >= S[idx+1]
    if idx + 1 < len(S):
        lam_hi = S[idx]       # just above threshold
        lam_lo = S[idx + 1]   # just below threshold
    else:
        lam_hi = S[idx]
        lam_lo = lam_hi * 0.5

    # choose k window so 1/k spans [lam_lo, lam_hi]
    k_lo = 1.0 / max(lam_hi, 1e-12)   # larger k -> smaller threshold
    k_hi = 1.0 / max(lam_lo, 1e-12)   # smaller k -> larger threshold
    # geometric mean as the evaluation point
    k_mid = float(np.sqrt(k_lo * k_hi))

    def N(lam):
        return max(np.sum(S > lam), 1.0)

    logN_hi = np.log(N(1.0 / k_lo))   # k small -> threshold large -> few above
    logN_lo = np.log(N(1.0 / k_hi))   # k large -> threshold small -> many above
    logk_hi = np.log(k_hi)
    logk_lo = np.log(k_lo)

    denom = logk_hi - logk_lo
    if abs(denom) < 1e-12:
        return 1.0

    C = (logN_hi - logN_lo) / denom
    C = float(np.clip(abs(C), 0.1, 50.0))
    return C

def compute_interdependence_exponent(weight_matrix: torch.Tensor, k: float,
                                     eps: float = None) -> float:
    """
    I(k) = |d log N(1/k) / d log k|

    Adaptive finite differences: window chosen from the local singular
    value spacing so that N(1/k) actually varies across the window.
    """
    if weight_matrix.numel() == 0:
        return 1.0
    ...
    k_lo = 1.0 / max(lam_hi, 1e-12)
    k_hi = 1.0 / max(lam_lo, 1e-12)

    def N(lam):
        return max(np.sum(S > lam), 1.0)

    logN_hi = np.log(N(1.0 / k_lo))
    logN_lo = np.log(N(1.0 / k_hi))
    logk_hi = np.log(k_hi)
    logk_lo = np.log(k_lo)
    ...
    I = -(logN_hi - logN_lo) / denom
    return float(np.clip(abs(I), 1e-4, 10.0))
...
def compute_hausdorff_dimension(weight: torch.Tensor, use_fast: bool = True) -> float:
    """
    Estimate the Hausdorff dimension of the weight matrix via the
    correlation dimension of its normalized singular-value distribution.

    D_corr = slope of log C(r) vs log r,
    where C(r) = fraction of pairs (i,j) with |p_i - p_j| < r
    and p_i = sigma_i / sum(sigma) are normalized singular values.

    This replaces the participation-ratio proxy, which saturates at the
    effective rank and was pinned at the ceiling for all weight matrices.

    DISCRETIZATION NOTE: the slope is estimated over the middle third of
    the log-log curve to avoid edge effects.
    """
    try:
        ...
        # Correlation integral C(r) = fraction of pairs closer than r
        # Computed efficiently via sorted distribution
        n = len(p)
        r_values = np.logspace(
            np.log10(max(p.min() * 1.5, 1e-12)),
            np.log10(max(p.max() * 0.5, 1e-9)),
            20
        )

        C = np.zeros(len(r_values))
        for k, r in enumerate(r_values):
            # cumulative count via broadcasting on sorted array
            # count_i = number of j with |p_i - p_j| < r
            diffs = np.abs(p[:, None] - p[None, :])
            C[k] = np.sum(diffs < r) / (n * n)

        # keep only positive C
        mask = C > 0
        if np.sum(mask) < 4:
            # fallback to participation ratio with wider ceiling
            D_fallback = 1.0 / np.sum(p ** 2)
            return float(np.clip(D_fallback, 0.05, 50.0))

        logr = np.log(r_values[mask])
        logC = np.log(C[mask])

        # slope over middle third
        lo = len(logr) // 3
        hi = 2 * len(logr) // 3
        if hi - lo < 2:
            lo, hi = 0, len(logr)
        ...
        D = float(np.median(slopes[lo:hi]))

        # realistic range for neural weight matrices is wider than [1.8, 14]
        # once the estimator is no longer saturating
        D = float(np.clip(D, 0.05, 50.0))
        return D
...
def check_thermodynamic_constraint(H_step: float, S_before: float, S_after: float,
    ...
    LANDAUER SCALING NOTE: The factor scale_factor (default 1.0) is a
...
class FGRFEngine:
    ...
    def process_layer(self, param: torch.nn.Parameter, param_name: str,
        ...
        # ---- Fractal Flow Factors (matrix-derived k, self-contained) ----
        if self.use_fractal:
            # Matrix-derived k selection:
            # 1/k is a singular-value threshold, so the meaningful k-range
            # for a given matrix is [1/S_max, 1/S_min] (clamped in dynamic range).
            try:
                S = torch.linalg.svdvals(param.data.detach().float().cpu()).numpy()
                S_pos = S[S > 0]
            except Exception:
                S_pos = np.array([])

            if len(S_pos) > 0:
                k_min = 1.0 / float(S_pos.max())
                k_max = 1.0 / float(S_pos.min())
                # clamp dynamic range to avoid absurd spans
                k_max = min(k_max, k_min * 1e4)
                if k_max <= k_min:
                    k_max = k_min * 10.0
                # deterministic log-uniform midpoint per matrix
                k = float(np.exp(0.5 * (np.log(k_min) + np.log(k_max))))
            else:
                k = 1.0

            # Interpolate Q(k) from the pre-solved flow
            k_clamped = float(np.clip(k, k_values[0], k_values[-1]))
            Q_k = float(np.interp(np.log(k_clamped), np.log(k_values), Q))

            # I derived from the matrix spectrum
            I_k = compute_interdependence_exponent(param.data, k)
            ...
            # Derive scale factor from the equations
        ...
        if self.use_thermo:
            ...
            if violated:
                ...
                logger.warning(
                    f"THERMO REJECT: {param_name}  "
                    f"H={H_step:.6e}  required_H={required_H:.6e}  "
                    f"dS={entropy_after - entropy_before:.6e}  "
                    f"h_scale={h_scale:.3e}"
                )
                return False, {
    ...
    def run(self, train_texts: List[str], eval_texts: List[str], 
        ...
        logger.info("=" * 80)
        logger.info("FGRF MULTI-MODEL ENGINE")
        logger.info("=" * 80)
        ...
        # ---- Solve Fractal Flow ----
        if self.use_fractal:
            # Derive the effective k-range from the actual weight matrices:
            # k in [1/S_max, 1/S_min] across all matrices to be processed.
            k_mins = []
            k_maxs = []
            for _, p in params:
                try:
                    S = torch.linalg.svdvals(p.data.detach().float().cpu()).numpy()
                    Sp = S[S > 0]
                    if len(Sp) > 0:
                        k_mins.append(1.0 / float(Sp.max()))
                        k_maxs.append(1.0 / float(Sp.min()))
                except Exception:
                    continue

            if k_mins and k_maxs:
                k_IR_eff = float(min(k_mins))
                k_UV_eff = float(max(k_maxs))
                # safety clamp on total dynamic range
                k_UV_eff = min(k_UV_eff, k_IR_eff * 1e6)
                if k_UV_eff <= k_IR_eff:
                    k_UV_eff = k_IR_eff * 10.0
            else:
                k_IR_eff = max(self.config.k_IR, 1e-10)
                k_UV_eff = max(self.config.k_UV, 1e-10)

            logger.info(f"    Effective k-range: [{k_IR_eff:.3e}, {k_UV_eff:.3e}] "
                        f"(matrix-derived, {len(k_mins)} matrices)")

            # Build a representative C_func from the first matrix
            ...
            logger.info(f"    Fractal Flow: Q_IR={Q[0]:.4f}, Q_UV={Q[-1]:.4f}")
            logger.info(f"    Interdependence (flow): informational only")
        else:
        ...
        output_path = os.path.join(self.config.output_dir, 
                                   f"multi_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
...
def main():
    ...
    logger.info("\n" + "=" * 80)
    logger.info("✓ FGRF MULTI-MODEL DEPLOYMENT COMPLETE")
    logger.info("=" * 80)
